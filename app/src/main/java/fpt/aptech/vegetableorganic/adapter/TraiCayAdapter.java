...
public class TraiCayAdapter  extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    Context context;
    List<Product> array;
    private static final int VIEW_TYPE_DATA=0;
    private static final int VIEW_TYPE_LOADING=1;

    public TraiCayAdapter(Context context, List<Product> array) {
        super();
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if(viewType==VIEW_TYPE_DATA){
            ...
            return new TraiCayAdapter.MyViewHolder(view);
        }else {
            View view=LayoutInflater.from(parent.getContext()).inflate(R.layout.item_loading,parent,false);
            return new TraiCayAdapter.LoadingViewHolder(view);
        }
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof TraiCayAdapter.MyViewHolder){
           TraiCayAdapter.MyViewHolder myViewHolder=(TraiCayAdapter.MyViewHolder) holder;
            Product product = array.get(position);
            myViewHolder.tensp.setText(product.getProduct_name());
            DecimalFormat decimalFormat=new DecimalFormat("###,###,###");

            myViewHolder.giasp.setText("Gía: "+decimalFormat.format(Double.parseDouble(product.getPrice()))+"Đ");
...
//            Glide.with(context).load(product.getProduct_image()).into(myViewHolder.hinhanh);
            ...
            TraiCayAdapter.LoadingViewHolder loadingViewHolder=(TraiCayAdapter.LoadingViewHolder) holder;
